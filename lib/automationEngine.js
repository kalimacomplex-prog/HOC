...
function getStepLibrary() {
  if (!STEP_LIBRARY) STEP_LIBRARY = require('./stepLibrary')({
    sub, fetch: M.fetch, Credencial: M.Credencial,
  });
  return STEP_LIBRARY;
}
...
async function escolherMaquina(empresa, roboId, runId) {
  const robo = roboId ? await M.Robot.findById(roboId).lean() : null;
  // "Nuvem": máquina da Nuvem da empresa (GitHub Actions), nunca máquina física.
  if (robo?.ambiente === 'nuvem') {
    // Enquanto espera na fila da Nuvem, a execução aparece como "Na fila" na aba Execuções.
    const marcar = (status) => async () => {
      const run = runId ? await M.AutomacaoRun.findById(runId).select('execucaoRoboId').lean() : null;
      if (run?.execucaoRoboId) await M.ExecucaoRobo.updateOne({ _id: run.execucaoRoboId, status: { $in: ['em_execucao', 'na_fila'] } }, { $set: { status } });
    };
    return obterMaquinaEfemera(empresa, {
      deveCancelar: runId ? () => isCancelled(runId) : null,
      aoEntrarNaFila: marcar('na_fila'),
      aoSairDaFila: marcar('em_execucao'),
    });
  }
  // "Local" (padrão): Agent Python numa máquina física da empresa, sem limite de
  // execuções simultâneas ("busy" é só rótulo visual do heartbeat). Sem Agent
  // online a execução falha — robô local nunca vai para a Nuvem sozinho.
  let maquina = null;
  if (robo?.maquinaId) {
    maquina = await M.Maquina.findOne({ _id: robo.maquinaId, empresa, status: { $in: ['online', 'busy'] }, ativo: true, efemera: { $ne: true } });
  }
  if (!maquina) {
  ...
  if (!maquina) {
    throw new Error('Nenhum Agent local online para rodar este passo. Ligue o Agent (HOC Agent) numa máquina da empresa ou mude o robô para "Rodar: Nuvem".');
  }
  return maquina;
}

// ==================== Máquina da Nuvem: UMA por empresa ====================
//
// Regra: no máximo UMA máquina da Nuvem (VM do GitHub Actions) por empresa por
// vez, rodando UMA execução por vez (EFEMERA_CAPACIDADE, padrão 1). As outras
// esperam numa fila FIFO e reaproveitam a mesma VM. A VM só fica ligada
// enquanto houver execução rodando ou alguém na fila — nunca ociosa: quando a
// última execução sai e a fila está vazia, a máquina é apagada na hora e o
// próximo heartbeat do runner recebe 401 e ele encerra sozinho.
//
// Quem garante "uma por empresa" é o índice único parcial { empresa } em Maquina
// (efemera:true, encerrando:false — ver index.js): duas execuções que chegam
// juntas não conseguem criar duas máquinas. Na própria Maquina:
// `usos` = execuções rodando nela (robosAtivos não serve: o heartbeat sobrescreve)
// e `filaOrdem` = senhas de quem espera, em ordem de chegada. Só a 1ª senha
// entra (com $pop no mesmo update); quem chega com fila não vazia vai para o
// fim dela, mesmo que haja vaga naquele instante — é isso que garante o FIFO.
const EFEMERA_CAPACIDADE = Math.max(1, parseInt(process.env.EFEMERA_CAPACIDADE, 10) || 1);
const EFEMERA_ESPERA_MS = Math.max(1, parseInt(process.env.EFEMERA_ESPERA_MINUTOS, 10) || 60) * 60 * 1000;
const EFEMERA_POLL_FILA_MS = 1000;
// Fila "abandonada": se a máquina está sem uso e ninguém da fila entrou nesse
// tempo, quem esperava morreu (ex.: servidor reiniciou) — a faxina apaga assim mesmo.
const EFEMERA_FILA_ABANDONADA_MS = 30 * 1000;
const FILA_VAZIA = { 'filaOrdem.0': { $exists: false } };

// opts: esperar (false = falha se ocupada), deveCancelar (async () => bool),
// aoEntrarNaFila / aoSairDaFila (avisos para marcar a execução como "na fila").
async function obterMaquinaEfemera(empresa, { esperar = true, deveCancelar = null, aoEntrarNaFila = null, aoSairDaFila = null } = {}) {
  const prazo = Date.now() + EFEMERA_ESPERA_MS;
  const senha = crypto.randomBytes(8).toString('hex');
  let naFilaDe = null; // _id da máquina em cuja fila esta senha está
  const sairDaFila = async () => {
    if (!naFilaDe) return null;
    const id = naFilaDe;
    naFilaDe = null;
    await M.Maquina.updateOne({ _id: id }, { $pull: { filaOrdem: senha } });
    if (aoSairDaFila) await aoSairDaFila().catch(() => {});
    return id;
  };
  try {
    for (;;) {
      // 1) Entrar numa vaga: sem fila, só se a fila estiver vazia; na fila, só
      // sendo a 1ª senha (e ela sai da fila no mesmo update).
      const filtroVaga = { empresa, efemera: true, encerrando: false, ativo: true, usos: { $lt: EFEMERA_CAPACIDADE } };
      const existente = naFilaDe
        ? await M.Maquina.findOneAndUpdate({ ...filtroVaga, _id: naFilaDe, 'filaOrdem.0': senha }, { $inc: { usos: 1 }, $pop: { filaOrdem: -1 }, $set: { ociosaDesde: null } }, { new: true })
        : await M.Maquina.findOneAndUpdate({ ...filtroVaga, ...FILA_VAZIA }, { $inc: { usos: 1 }, $set: { ociosaDesde: null } }, { new: true });
      if (existente) {
        if (naFilaDe) { naFilaDe = null; if (aoSairDaFila) await aoSairDaFila().catch(() => {}); }
        return existente;
      }

      const atual = await M.Maquina.findOne({ empresa, efemera: true, encerrando: false }).select('_id filaOrdem').lean();
      if (!atual) {
        // 2) Não existe: cria. Se outra execução criar no mesmo instante, o índice
        // único recusa esta (E11000) e a volta do laço entra na fila dela.
        await sairDaFila(); // a máquina em cuja fila estava já não existe
        const machineKey = crypto.randomBytes(24).toString('hex');
        let maquina;
        try {
          maquina = await M.Maquina.create({
            nome: 'Máquina da Nuvem (GitHub Actions)',
            machineId: `ephemeral-${Date.now()}-${crypto.randomBytes(4).toString('hex')}`,
            machineKey,
            capacidadeMaxima: EFEMERA_CAPACIDADE,
      ...
      // 3) Existe e está ocupada (ou já tem fila): entra no fim da fila dela.
      if (!esperar) throw new Error('A máquina da Nuvem da empresa está ocupada com outra execução (uma por vez). Tente de novo em instantes.');
      if (String(naFilaDe) !== String(atual._id)) {
        await sairDaFila();
        await M.Maquina.updateOne({ _id: atual._id }, { $push: { filaOrdem: senha } });
        naFilaDe = atual._id;
        if (aoEntrarNaFila) await aoEntrarNaFila().catch(() => {});
      }
      if (Date.now() > prazo) throw new Error('Tempo esgotado esperando vaga na máquina da Nuvem da empresa.');
      if (deveCancelar && await deveCancelar()) throw Object.assign(new Error('Execução cancelada'), { cancelled: true });
      await sleep(EFEMERA_POLL_FILA_MS);
    }
  } catch (e) {
    // Desistiu (cancelou, prazo, erro): sai da fila e, se era só ela segurando a
    // máquina sem uso, apaga agora.
    const id = await sairDaFila();
    if (id) await encerrarMaquinaEfemeraSeOciosa(id).catch(() => {});
    throw e;
  }
}

// Uma execução terminou de usar a máquina da Nuvem. Se era a última e ninguém
// está na fila, a máquina é apagada na hora (o runner recebe 401 e encerra).
async function liberarMaquinaEfemera(maquinaId) {
  const m = await M.Maquina.findOneAndUpdate({ _id: maquinaId, efemera: true, usos: { $gt: 0 } }, { $inc: { usos: -1 } }, { new: true });
  if (m && m.usos <= 0) {
    await M.Maquina.updateOne({ _id: maquinaId, usos: { $lte: 0 } }, { $set: { ociosaDesde: new Date() } });
    await encerrarMaquinaEfemeraSeOciosa(maquinaId);
  }
}

// Apaga a máquina se ninguém estiver usando nem esperando — tudo no mesmo update,
// então uma execução que entrou ou se registrou na fila nesse meio tempo impede o
// encerramento. Com `abandonadas`, ignora a fila se a máquina está sem uso há mais
// que EFEMERA_FILA_ABANDONADA_MS (quem esperava morreu com o servidor).
async function encerrarMaquinaEfemeraSeOciosa(maquinaId, { abandonadas = false } = {}) {
  const filtro = { _id: maquinaId, efemera: true, encerrando: false, usos: { $lte: 0 } };
  if (abandonadas) filtro.ociosaDesde = { $ne: null, $lte: new Date(Date.now() - EFEMERA_FILA_ABANDONADA_MS) };
  else Object.assign(filtro, FILA_VAZIA);
  const m = await M.Maquina.findOneAndUpdate(filtro, { $set: { encerrando: true } });
  if (m) await M.Maquina.findByIdAndDelete(maquinaId);
  return !!m;
}
...
// ==================== Sessões de IA ====================
//
// Mesmo modelo das sessões de navegador: `ai_open` guarda em ctx.aiSessions[nome]
// o provedor, o token e as configurações; `ai_chat`/`ai_close` e as ações OpenAI
// só citam o nome. Vive na memória do run (some quando ele termina) e roda no
// servidor — o token nunca é enviado à máquina do agente.

const AI_PROVIDERS = ['claude-sonnet', 'claude-haiku', 'gpt-4o', 'gpt-4', 'gemini', 'groq'];
const AI_OPENAI_PROVIDERS = ['gpt-4o', 'gpt-4'];
// Ações que chamam a OpenAI com `api_key` e aceitam `session_name` no lugar dela.
// (semantic_search fica de fora: é só similaridade local, não chama API nenhuma.)
const AI_OPENAI_SESSION_TYPES = new Set([
  'generate_embedding', 'moderate_content', 'generate_ai_image', 'transcribe_audio', 'text_to_speech',
]);

function getAiSession(name, ctx) {
  const session = ctx.aiSessions?.[name];
  if (!session) {
    throw new Error(`Sessão de IA "${name}" não encontrada. Adicione um step "Abrir sessão de IA" com esse nome antes deste.`);
  }
  return session;
}

// Se a ação OpenAI tem `session_name`, devolve uma cópia da config com o api_key
// da sessão (a config salva no robô não é alterada).
function aplicarSessaoIANaConfig(tipo, cfg, ctx) {
  if (!AI_OPENAI_SESSION_TYPES.has(tipo)) return cfg;
  const nome = sub(String(cfg.session_name || ''), ctx).trim();
  if (!nome) return cfg;
  const session = getAiSession(nome, ctx);
  if (!AI_OPENAI_PROVIDERS.includes(session.provedor)) {
    throw new Error(`A sessão "${nome}" usa o provedor "${session.provedor}", mas esta ação só funciona com uma sessão OpenAI (GPT-4o / GPT-4).`);
  }
  return { ...cfg, api_key: session.apiKey };
}

async function aiOpen(cfg, ctx, empresa) {
  const nome = sub(String(cfg.session_name || ''), ctx).trim();
  if (!nome) throw new Error('Informe um nome para a sessão de IA no campo "Nome da sessão".');
  ctx.aiSessions ||= {};
  if (ctx.aiSessions[nome]) return `Sessão de IA "${nome}" já está aberta — reaproveitando.`;

  let provedor = String(cfg.provedor || '').trim();
  let credNome = String(cfg.credencial_nome || '').trim();
  let campo = String(cfg.campo_cred || '').trim() || 'api_key';
  const tokenDigitado = sub(String(cfg.api_key || ''), ctx).trim();
  if (!provedor || provedor === 'empresa') {
    // "Padrão da empresa": provedor e credencial de Configurações → IA.
    const padrao = await M.ConfigIA.findOne({ empresa }).lean();
    provedor = padrao?.provedor || 'claude-sonnet';
    if (!tokenDigitado && !credNome) { credNome = padrao?.credencialNome || ''; campo = padrao?.campoCred || 'api_key'; }
  }
  if (!AI_PROVIDERS.includes(provedor)) throw new Error(`Provedor de IA inválido: "${provedor}".`);

  let apiKey = tokenDigitado;
  if (!apiKey) {
    if (!credNome) throw new Error('Informe o token da sessão ou escolha uma credencial do cofre.');
    const cred = await M.Credencial.findOne({ nome: credNome, empresa }).lean();
    apiKey = cred?.campos?.[campo] || '';
    if (!apiKey) throw new Error(`Credencial "${credNome}" não encontrada ou sem o campo "${campo}".`);
  }

  const num = (v) => (v === '' || v == null || Number.isNaN(Number(v)) ? null : Number(v));
  ctx.aiSessions[nome] = {
    provedor, apiKey,
    modelo: sub(String(cfg.modelo || ''), ctx).trim() || null, // opcional: sobrepõe o modelo padrão do provedor
    systemMessage: sub(String(cfg.system_message || ''), ctx),
    temperature: num(cfg.temperature),
    maxTokens: num(cfg.max_tokens) ? Math.round(num(cfg.max_tokens)) : null,
    keepHistory: cfg.keep_history !== false,
    maxHistory: Math.max(2, parseInt(cfg.max_history_messages, 10) || 20),
    history: [],
  };
  const memoria = ctx.aiSessions[nome].keepHistory ? ' — com memória da conversa' : ' — sem memória (cada mensagem é independente)';
  const modeloTxt = ctx.aiSessions[nome].modelo ? ` / ${ctx.aiSessions[nome].modelo}` : '';
  return `Sessão de IA "${nome}" aberta (${provedor}${modeloTxt})${memoria}.`;
}

async function aiChat(cfg, ctx, empresa) {
  const nome = sub(String(cfg.session_name || ''), ctx).trim();
  if (!nome) throw new Error('Informe o nome da sessão de IA no campo "Nome da sessão".');
  const session = getAiSession(nome, ctx);
  const mensagem = sub(cfg.input_template || '{output}', ctx);
  const { resposta } = await M.resolverEChamarIA(empresa, {
    userMessage: mensagem,
    systemMessage: session.systemMessage || undefined,
    provedor: session.provedor,
    modelo: session.modelo || undefined,
    apiKey: session.apiKey,
    history: session.keepHistory ? session.history : undefined,
    temperature: session.temperature ?? undefined,
    maxTokens: session.maxTokens ?? undefined,
  });
  if (session.keepHistory) {
    session.history.push({ role: 'user', content: mensagem }, { role: 'assistant', content: resposta });
    const excesso = session.history.length - session.maxHistory;
    // descarta as mais antigas em pares, pra conversa nunca começar pela resposta do assistente
    if (excesso > 0) session.history.splice(0, excesso + (excesso % 2));
  }
  return resposta;
}
...
async function execStepServer(step, ctx, empresa) {
  ...
  if (t === 'ai_open') {
    const msg = await aiOpen(cfg, ctx, empresa);
    store(msg, cfg.variable_name, ctx);
    return msg;
  }

  if (t === 'ai_chat') {
    const resposta = await aiChat(cfg, ctx, empresa);
    store(resposta, cfg.variable_name, ctx);
    return resposta;
  }
  ...
  if (t === 'ai_close') {
    ...
    if (!nome) throw new Error('Informe o nome da sessão de IA no campo "Nome da sessão".');
    const session = getAiSession(nome, ctx);
    const descartadas = session.history.length;
    delete ctx.aiSessions[nome];
    const msg = `Sessão de IA "${nome}" encerrada` + (descartadas ? ` (${descartadas} mensagens descartadas).` : '.');
    store(msg, cfg.variable_name, ctx);
    return msg;
  }
  ...
  if (lib[t]) {
    const resultado = await lib[t](aplicarSessaoIANaConfig(t, cfg, ctx), ctx, empresa);
...
async function execLeafStep(step, ctx, empresa, maquinaIdRef) {
  if (AGENT_STEP_TYPES.has(step.type)) {
    if (!maquinaIdRef.current) {
      const maquina = await escolherMaquina(empresa, ctx._roboId, ctx._runId);
...
async function _rodarLoop(automacao, run, empresa, roboId) {
  // aiSessions criado aqui (não sob demanda) pra ser o MESMO objeto nas cópias rasas
  // de ctx que o walker faz nos ramos paralelos.
  const ctx = { input: run.input || '', output: run.input || '', vars: {}, aiSessions: {}, _runId: run._id, _roboId: roboId || automacao.roboId };
  const results = [];
  ...
  if (maquinaIdRef.current) {
    ...
    if (maq?.efemera) {
      // Máquina da Nuvem é compartilhada pela empresa: sai dela; a última
      // execução a sair apaga (o runner recebe 401 e encerra sozinho).
      await liberarMaquinaEfemera(maquinaIdRef.current);
    } else {
      await M.Maquina.findByIdAndUpdate(maquinaIdRef.current, { $inc: { robosAtivos: -1 } });
    }
  }
...
module.exports = { init, iniciarRunEmBackground, executarESperar, obterMaquinaEfemera, liberarMaquinaEfemera, encerrarMaquinaEfemeraSeOciosa };
