...
async function escolherMaquina(empresa, roboId, runId) {
  ...
  if (robo?.ambiente === 'nuvem') return obterMaquinaEfemera(empresa, { runId });
...
// esperam na fila e reaproveitam a mesma VM. A VM só fica ligada enquanto houver
// execução rodando ou alguém na fila — nunca ociosa: quando a última execução
// sai e a fila está vazia, a máquina é apagada na hora e o próximo heartbeat do
// runner recebe 401 e ele encerra sozinho.
//
...
// juntas não conseguem criar duas máquinas. Contadores na própria Maquina:
// `usos` = execuções rodando nela (robosAtivos não serve: o heartbeat sobrescreve)
// e `fila` = execuções esperando vaga nela.
...
async function obterMaquinaEfemera(empresa, { runId = null, esperar = true } = {}) {
  const prazo = Date.now() + EFEMERA_ESPERA_MS;
  let naFilaDe = null; // _id da máquina em cuja fila esta execução se registrou
  const sairDaFila = async () => {
    if (!naFilaDe) return null;
    const id = naFilaDe;
    naFilaDe = null;
    await M.Maquina.updateOne({ _id: id, fila: { $gt: 0 } }, { $inc: { fila: -1 } });
    return id;
  };
  try {
    for (;;) {
      // 1) Entra na máquina da empresa que já está de pé, se tiver vaga.
      const existente = await M.Maquina.findOneAndUpdate(
        { empresa, efemera: true, encerrando: false, ativo: true, usos: { $lt: EFEMERA_CAPACIDADE } },
        { $inc: { usos: 1 }, $set: { ociosaDesde: null } },
        { new: true },
      );
      if (existente) { await sairDaFila(); return existente; }

      const atual = await M.Maquina.findOne({ empresa, efemera: true, encerrando: false }).select('_id').lean();
      if (!atual) {
        // 2) Não existe: cria. Se outra execução criar no mesmo instante, o índice
        // único recusa esta (E11000) e a volta do laço entra na dela.
      ...
      // 3) Existe e está cheia: entra na fila dela (robô do builder) ou avisa (Git/ZIP).
      ...
      if (String(naFilaDe) !== String(atual._id)) {
        await sairDaFila();
        await M.Maquina.updateOne({ _id: atual._id }, { $inc: { fila: 1 } });
        naFilaDe = atual._id;
      }
      if (Date.now() > prazo) throw new Error('Tempo esgotado esperando vaga na máquina da Nuvem da empresa.');
      if (runId && await isCancelled(runId)) throw Object.assign(new Error('Execução cancelada'), { cancelled: true });
