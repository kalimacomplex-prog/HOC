...
module.exports = function buildStepLibrary({ sub, fetch, Credencial }) {
  ...
  // ==================== Google Drive ====================
  // Autenticação via conta de serviço do Google (JSON colado no cofre de
  // Credenciais ou digitado no próprio step) — sem fluxo de login/consentimento,
  // já que o motor de automação roda sem interação humana. O dono compartilha
  // uma pasta do Drive com o e-mail "client_email" da conta de serviço pra
  // liberar o acesso (a conta de serviço não tem "Meu Drive" próprio com cota).
  async function gdriveResolveServiceAccountJson(cfg, ctx, empresa) {
    const digitado = sub(String(cfg.api_key || ''), ctx).trim();
    if (digitado) return digitado;
    const credNome = String(cfg.credencial_nome || '').trim();
    if (!credNome) throw new Error('Informe o JSON da conta de serviço do Google ou escolha uma credencial do cofre.');
    ...
    if (!valor) throw new Error(`Credencial "${credNome}" não encontrada ou sem o campo "${campo}" (cole ali o JSON da conta de serviço do Google).`);
    return valor;
  }

  async function gdriveToken(cfg, ctx, empresa) {
    ctx._gdriveTokens ||= {};
    const saJsonRaw = await gdriveResolveServiceAccountJson(cfg, ctx, empresa);
    const cacheKey = crypto.createHash('sha1').update(saJsonRaw).digest('hex');
    const cached = ctx._gdriveTokens[cacheKey];
    if (cached && cached.expiresAt > Date.now() + 30000) return cached.token;

    let sa;
    try { sa = JSON.parse(saJsonRaw); } catch { throw new Error('JSON da conta de serviço do Google inválido — cole o arquivo de chave completo (Google Cloud → IAM e admin → Contas de serviço → Chaves).'); }
