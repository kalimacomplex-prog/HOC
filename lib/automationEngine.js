...
let M = null; // { Automacao, AutomacaoRun, AutomacaoStepDispatch, Robot, ExecucaoRobo, Maquina, enviarEmail, resolverEChamarIA, fetch }
...
const AGENT_STEP_TYPES = new Set(['read_file', 'write_file', 'run_command', 'browser_flow']);
const STEP_DISPATCH_TIMEOUT_MS = 5 * 60 * 1000;
...
async function execStepServer(step, ctx, empresa) {
  ...
  throw new Error(`Tipo de step desconhecido ou não implementado no MVP: ${t}`);
}
