...
async function escolherMaquina(empresa, roboId, runId) {
  ...
  if (roboId) {
    ...
    // Robô configurado como "Nuvem (GitHub Actions)": nunca usa máquina física
    // do tenant, mesmo que haja uma online — vai direto pra máquina da Nuvem.
  ...
  if (!maquina) {
    maquina = await M.Maquina.findOne({
      empresa, status: 'online', ativo: true, efemera: { $ne: true },
      $expr: { $lt: ['$robosAtivos', '$capacidadeMaxima'] },
    }).sort({ robosAtivos: 1 });
  }
  // Nenhuma máquina física do tenant disponível — usa a máquina da Nuvem
  // (runner do GitHub Actions) da empresa. Ela se registra como uma Maquina
  // temporária (machineKey só dela) fazendo heartbeat normalmente —
  // reaproveita 100% do protocolo de dispatch de step já existente.
  if (!maquina) {
    maquina = await obterMaquinaEfemera(empresa, { runId });
  }
  return maquina;
}
...
//
// Todas as execuções da empresa que precisam da Nuvem ao mesmo tempo dividem o
// mesmo runner do GitHub Actions (nunca mais de uma VM por empresa). Quem
...
const EFEMERA_CAPACIDADE = Math.max(1, parseInt(process.env.EFEMERA_CAPACIDADE, 10) || 4);
const EFEMERA_ESPERA_MS = 30 * 60 * 1000;
...
async function obterMaquinaEfemera(empresa, { runId = null, esperar = true } = {}) {
  ...
  for (;;) {
    ...
    if (!esperar) throw new Error(`A máquina da Nuvem da empresa está ocupada (${EFEMERA_CAPACIDADE} execuções ao mesmo tempo). Tente de novo em instantes.`);
