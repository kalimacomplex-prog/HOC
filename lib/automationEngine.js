...
function getStepLibrary() {
  if (!STEP_LIBRARY) STEP_LIBRARY = require('./stepLibrary')({
    ...
    googleOAuthClientId: M.googleOAuthClientId, googleOAuthClientSecret: M.googleOAuthClientSecret,
  });
  return STEP_LIBRARY;
}
...
async function escolherMaquina(empresa, roboId) {
  ...
  if (roboId) {
    ...
    // do tenant, mesmo que haja uma online — vai direto pro runner efêmero.
    if (robo?.ambiente === 'nuvem') return criarMaquinaEfemera(empresa);
    if (robo?.maquinaId) {
...
module.exports = { init, iniciarRunEmBackground, executarESperar, criarMaquinaEfemera };
