...
module.exports = function buildStepLibrary({ sub, fetch, Credencial }) {
  ...
  // ==================== Google Drive ====================
  // Duas formas de autenticar, resolvidas pela credencial do cofre (ou pelo
  // JSON colado no step) — o conteúdo decide qual é usada:
  //  1) OAuth com o app da PRÓPRIA empresa (credencial com client_id,
  //     client_secret e refresh_token — gerada por "Conectar conta Google" em
  //     Operações → Credenciais): acessa o Drive da conta Google que autorizou,
  //     com a cota dela — funciona pra tudo, inclusive enviar/atualizar arquivo.
  //  2) Conta de serviço (JSON com client_email/private_key): sem login, mas
  ...
  // Devolve { tipo: 'oauth', client_id, client_secret, refresh_token } ou { tipo: 'sa', sa }.
  function gdriveInterpretar(obj, origem) {
    if (obj && obj.client_id && obj.client_secret && obj.refresh_token) {
      return { tipo: 'oauth', client_id: String(obj.client_id), client_secret: String(obj.client_secret), refresh_token: String(obj.refresh_token) };
    }
    if (obj && obj.client_email) return { tipo: 'sa', sa: obj };
    if (obj && obj.client_id && obj.client_secret) throw new Error(`${origem}: a conta Google ainda não foi conectada — em Operações → Credenciais, clique em "Conectar conta Google" e autorize.`);
    throw new Error(`${origem}: esperado client_id + client_secret + refresh_token (conta Google conectada) ou o JSON de uma conta de serviço.`);
  }

  async function gdriveResolveCredencial(cfg, ctx, empresa) {
    const digitado = sub(String(cfg.api_key || ''), ctx).trim();
    if (digitado) return gdriveInterpretar(jparse(digitado, null), 'Credencial colada no step');
    const credNome = String(cfg.credencial_nome || '').trim();
    if (!credNome) throw new Error('Escolha a credencial do Google no step (Operações → Credenciais → "Conectar conta Google").');
    ...
    if (!cred) throw new Error(`Credencial "${credNome}" não encontrada.`);
    const campos = cred.campos || {};
    if (campos.client_id) return gdriveInterpretar(campos, `Credencial "${credNome}"`);
    // Sem client_id: conta de serviço. Um refresh_token solto aqui é de uma conexão
    // antiga pelo app do HOC (que não existe mais) — ignorado se houver o JSON.
    const campo = String(cfg.campo_cred || '').trim() || 'service_account_json';
    const valor = campos[campo] || '';
    if (!valor && campos.refresh_token) throw new Error(`Credencial "${credNome}" é de uma conexão antiga com o Google — reconecte em Operações → Credenciais → "Conectar conta Google".`);
    if (!valor) throw new Error(`Credencial "${credNome}" sem o campo "${campo}".`);
    return gdriveInterpretar(jparse(valor, null), `Credencial "${credNome}"`);
  }
  ...
  async function gdriveTokenViaOAuthRefresh(c) {
    return fetch('https://oauth2.googleapis.com/token', {
      method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
  ...
  async function gdriveToken(cfg, ctx, empresa) {
    ctx._gdriveTokens ||= {};
    const c = await gdriveResolveCredencial(cfg, ctx, empresa);
    const cacheKey = crypto.createHash('sha1').update(JSON.stringify(c)).digest('hex');
    const cached = ctx._gdriveTokens[cacheKey];
    if (cached && cached.expiresAt > Date.now() + 30000) return cached.token;

    const r = c.tipo === 'sa' ? await gdriveTokenViaServiceAccount(c.sa) : await gdriveTokenViaOAuthRefresh(c);
    const data = await r.json().catch(() => ({}));
    if (!r.ok || !data.access_token) {
      const motivo = data.error_description || data.error || `HTTP ${r.status}`;
      const dica = data.error === 'invalid_grant' ? ' — a autorização expirou ou foi revogada; reconecte a conta Google em Operações → Credenciais (se o app do Google Cloud estiver em modo "Teste", publique-o para a conexão não expirar em 7 dias).' : '';
      throw new Error('Falha ao autenticar no Google: ' + motivo + dica);
    }
  ...
  // ==================== Google Sheets ====================
  // Mesma credencial/autenticação do Google Drive acima (o escopo "drive"
  // também autoriza a API do Sheets — não precisa de um escopo separado).
  // Criar planilha nova consome cota de armazenamento (mesma limitação da
  // conta de serviço descrita acima); ler/escrever célula em planilha já
  // existente funciona com conta de serviço ou OAuth.
  async function gsheetsFetch(cfg, ctx, empresa, path, opts = {}) {
    const token = await gdriveToken(cfg, ctx, empresa);
    const r = await fetch(`https://sheets.googleapis.com${path}`, { ...opts, headers: { Authorization: `Bearer ${token}`, ...(opts.headers || {}) } });
    if (!r.ok) throw new Error(`Google Sheets: HTTP ${r.status} — ${(await r.text()).slice(0, 400)}`);
    return r;
  }

  // Aceita uma linha só (["a","b"]) ou várias ([["a","b"],["c","d"]]) no
  // mesmo campo "Dados (JSON)" — normaliza pra sempre virar lista de linhas.
  function gsheetsLinhas(dataInputRaw) {
    let parsed;
    try { parsed = JSON.parse(dataInputRaw); } catch { throw new Error('Dados inválidos — informe um JSON (uma linha ["a","b"] ou várias linhas [["a","b"],["c","d"]]).'); }
    if (!Array.isArray(parsed)) throw new Error('Dados inválidos — informe um array JSON.');
    return Array.isArray(parsed[0]) ? parsed : [parsed];
  }

  lib.gsheets_create_spreadsheet = async (cfg, ctx, empresa) => {
    const titulo = sub(cfg.gsheets_titulo || 'Nova planilha', ctx);
    const r = await gsheetsFetch(cfg, ctx, empresa, '/v4/spreadsheets', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ properties: { title: titulo } }),
    });
    const data = await r.json();
    const pastaId = sub(cfg.gdrive_parent_id || '', ctx).trim();
    if (pastaId) await gdriveFetch(cfg, ctx, empresa, `/drive/v3/files/${encodeURIComponent(data.spreadsheetId)}?addParents=${encodeURIComponent(pastaId)}`, { method: 'PATCH' });
    return JSON.stringify({ id: data.spreadsheetId, url: data.spreadsheetUrl, title: titulo });
  };

  lib.gsheets_read_values = async (cfg, ctx, empresa) => {
    const id = sub(cfg.gsheets_id || '', ctx).trim();
    const range = sub(cfg.gsheets_range || '', ctx).trim() || 'A1:Z1000';
    if (!id) throw new Error('Informe o ID da planilha.');
    const r = await gsheetsFetch(cfg, ctx, empresa, `/v4/spreadsheets/${encodeURIComponent(id)}/values/${encodeURIComponent(range)}`);
    return await r.text();
  };

  lib.gsheets_write_values = async (cfg, ctx, empresa) => {
    const id = sub(cfg.gsheets_id || '', ctx).trim();
    const range = sub(cfg.gsheets_range || '', ctx).trim();
    if (!id || !range) throw new Error('Informe o ID da planilha e o intervalo (ex.: "Página1!A1").');
    const values = gsheetsLinhas(sub(cfg.data_input || '[]', ctx));
    const r = await gsheetsFetch(cfg, ctx, empresa, `/v4/spreadsheets/${encodeURIComponent(id)}/values/${encodeURIComponent(range)}?valueInputOption=USER_ENTERED`, {
      method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ values }),
    });
    return await r.text();
  };

  lib.gsheets_append_row = async (cfg, ctx, empresa) => {
    const id = sub(cfg.gsheets_id || '', ctx).trim();
    const range = sub(cfg.gsheets_range || '', ctx).trim() || 'A1';
    if (!id) throw new Error('Informe o ID da planilha.');
    const values = gsheetsLinhas(sub(cfg.data_input || '[]', ctx));
    const r = await gsheetsFetch(cfg, ctx, empresa, `/v4/spreadsheets/${encodeURIComponent(id)}/values/${encodeURIComponent(range)}:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ values }),
    });
    return await r.text();
  };

  lib.gsheets_clear_values = async (cfg, ctx, empresa) => {
    const id = sub(cfg.gsheets_id || '', ctx).trim();
    const range = sub(cfg.gsheets_range || '', ctx).trim();
    if (!id || !range) throw new Error('Informe o ID da planilha e o intervalo.');
    const r = await gsheetsFetch(cfg, ctx, empresa, `/v4/spreadsheets/${encodeURIComponent(id)}/values/${encodeURIComponent(range)}:clear`, { method: 'POST' });
    return await r.text();
  };

  lib.gsheets_list_sheets = async (cfg, ctx, empresa) => {
    const id = sub(cfg.gsheets_id || '', ctx).trim();
    if (!id) throw new Error('Informe o ID da planilha.');
    const r = await gsheetsFetch(cfg, ctx, empresa, `/v4/spreadsheets/${encodeURIComponent(id)}?fields=sheets.properties`);
    return await r.text();
  };

  lib.gsheets_add_sheet = async (cfg, ctx, empresa) => {
    const id = sub(cfg.gsheets_id || '', ctx).trim();
    const titulo = sub(cfg.gsheets_titulo || '', ctx).trim();
    if (!id || !titulo) throw new Error('Informe o ID da planilha e o nome da aba.');
    const r = await gsheetsFetch(cfg, ctx, empresa, `/v4/spreadsheets/${encodeURIComponent(id)}:batchUpdate`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  lib.gsheets_delete_sheet = async (cfg, ctx, empresa) => {
    const id = sub(cfg.gsheets_id || '', ctx).trim();
    const titulo = sub(cfg.gsheets_titulo || '', ctx).trim();
    if (!id || !titulo) throw new Error('Informe o ID da planilha e o nome da aba.');
    const metaR = await gsheetsFetch(cfg, ctx, empresa, `/v4/spreadsheets/${encodeURIComponent(id)}?fields=sheets.properties`);
    const meta = await metaR.json();
    const aba = (meta.sheets || []).find((s) => s.properties?.title === titulo);
    if (!aba) throw new Error(`Aba "${titulo}" não encontrada nessa planilha.`);
    const r = await gsheetsFetch(cfg, ctx, empresa, `/v4/spreadsheets/${encodeURIComponent(id)}:batchUpdate`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ requests: [{ deleteSheet: { sheetId: aba.properties.sheetId } }] }),
    });
    return await r.text();
  };
  ...
  // O servidor do HOC é compartilhado por todas as empresas e o process.env dele
  // guarda segredos (JWT_SECRET, MONGO_URI, chaves de pagamento…): só nomes
  // inofensivos desta lista podem ser lidos por um robô.
  const ENV_PERMITIDAS = new Set(['NODE_ENV', 'TZ']);
  lib.read_env_var = async (cfg, ctx) => {
    const nome = sub(cfg.value || '', ctx).trim();
    if (!ENV_PERMITIDAS.has(nome)) {
      throw new Error(`Variável "${nome}" não permitida. Só é possível ler: ${[...ENV_PERMITIDAS].join(', ')}. Para guardar valores próprios, use o cofre de Credenciais.`);
    }
