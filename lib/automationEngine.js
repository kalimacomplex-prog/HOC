...
// esperam na fila e reaproveitam a mesma VM enquanto ela estiver de pé. Quem
// garante isso é o índice único parcial { empresa } em Maquina (efemera:true,
// encerrando:false — ver index.js): duas execuções que chegam juntas não
// conseguem criar duas máquinas. `usos` conta as execuções usando a máquina
// (não dá pra usar robosAtivos: o heartbeat sobrescreve com o número que o
// runner informa). Quando `usos` volta a 0 a máquina é apagada e o próximo
// heartbeat do runner recebe 401 e encerra sozinho, como antes.
...
// Carência: quando a última execução sai, a máquina fica ligada ociosa por um
// tempo antes de ser apagada — quem está na fila ou chega logo depois reaproveita
// o runner já de pé (subir outro custa ~50 s no GitHub). Tem que ser menor que o
// IDLE_TIMEOUT_SECONDS do runner (300 s), senão ele se desliga antes.
const EFEMERA_OCIOSA_MS = Math.max(0, parseInt(process.env.EFEMERA_OCIOSA_SEGUNDOS, 10) || 60) * 1000;
...
async function obterMaquinaEfemera(empresa, { runId = null, esperar = true } = {}) {
  ...
  for (;;) {
    ...
    // 2) Existe, mas está cheia: espera vaga (robô do builder) ou avisa (Git/ZIP).
    const cheia = await M.Maquina.exists({ empresa, efemera: true, encerrando: false });
    if (!cheia) {
      // 3) Não existe: cria. Se outra execução criar no mesmo instante, o índice
...
// Uma execução terminou de usar a máquina da Nuvem. Quando a última sai, a
// máquina fica ociosa pela carência e só então é apagada (o runner recebe 401 e
// encerra sozinho). Se o servidor reiniciar no meio, a faxina de 60 s do
// index.js chama encerrarMaquinaEfemeraSeOciosa e cobre.
async function liberarMaquinaEfemera(maquinaId) {
  ...
  if (m && m.usos <= 0) {
    ...
    setTimeout(() => encerrarMaquinaEfemeraSeOciosa(maquinaId).catch(() => {}), EFEMERA_OCIOSA_MS + 500);
  }
}

// Apaga a máquina só se ninguém estiver usando E ela estiver ociosa há pelo menos
// a carência — tudo no mesmo update, então uma execução que entrou nesse meio
// tempo (usos>0, ociosaDesde zerado) impede o encerramento.
async function encerrarMaquinaEfemeraSeOciosa(maquinaId) {
  const m = await M.Maquina.findOneAndUpdate(
    { _id: maquinaId, efemera: true, encerrando: false, usos: { $lte: 0 }, ociosaDesde: { $ne: null, $lte: new Date(Date.now() - EFEMERA_OCIOSA_MS) } },
    { $set: { encerrando: true } },
  );
